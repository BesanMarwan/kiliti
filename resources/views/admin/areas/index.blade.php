...
@section('title',lng('dashboard.areas.areas','المناطق'))
@section('page_title')
    <x-header.title :name="lng('dashboard.general.dashboard','لوحة التحكم')">
        <x-header.breadcrumb-item :name="lng('dashboard.areas.areas','المناطق')"/>
    </x-header.title>
@endsection
@push('css')
...
@section('search_content')
    <x-search :searchable="\App\Models\City::getSearchable()"/>
@endsection
@section('content')
    <x-theme.card>
        <x-slot name="toolbar">
            <a  class="btn btn-sm btn-info  mx-2 OpenModal"
                data-title="{{lng('dashboard.areas.create_areas','اضافة منطقة جديدة ')}}"
                href="{{route('system.areas.create')}}"
                data-size="modal-xl"
                level="2"
            >
                <span class="svg-icon svg-icon-white svg-icon-x">@svg('lineawesome-plus-solid')</span>
                <span>@lng('dashboard.areas.create_areas','اضافة منطقة جديدة ')</span>
            </a>
        </x-slot>
                    ...
                    <tr id="TR_{{$o->id}}">
                        ...
                        <td class="text-center">
                            ...
                            <a  data-title="{{lng('dashboard.general.cities','عرض مدن المنطقة ')}} "
                                data-size="modal-xl"
                                level="3"
                                href="{{route('system.areas.cities',['id'=>$o->id])}}" class="text-gray-800 text-hover-primary mb-1 OpenModal">
                        ...
                        <td class="text-center">
                            <x-theme.tables.menu :title="lng('dashboard.general.settings','الاعدادات')">
                                <x-theme.tables.menu-item :title="lng('dashboard.general.edit','تعديل')"
                                                          class="OpenModal"
                                                          data-title="{{lng('dashboard.general.edit_area','تعديل بيانات منطقة')}} "
                                                          data-size="modal-xl"
                                                          level="2"
                                                          :href="route('system.areas.update',$o->id)"/>

                                @if($o->childs->count())

                                    <x-theme.tables.menu-item :title="lng('dashboard.areas.all_cities','عرض المدن')"
                                                              class="OpenModal"
                                                              data-title="{{lng('dashboard.areas.cities','عرض مدن المنطقة')}} "
                                                              data-size="modal-xl"
                                                              level="2"
                                                              :href="route('system.areas.cities',$o->id)"/>
                                @endif
