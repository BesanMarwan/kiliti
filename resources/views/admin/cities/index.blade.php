...
@section('page_title')
    <x-header.title :name="lng('dashboard.general.dashboard','لوحة التحكم')">
        <x-header.breadcrumb-item :name="lng('dashboard.cities.cities','المدن')"/>
    </x-header.title>
@endsection
@push('css')
...
@section('search_content')
    <x-search :searchable="\App\Models\City::getSearch()"/>
@endsection
@section('content')
    <x-theme.card>
        <x-slot name="toolbar">
            <a  class="btn btn-sm btn-info  mx-2 OpenModal"
                data-title="{{lng('dashboard.areas.create_city','اضافة مدينة')}}"
                href="{{route('system.cities.create')}}"
                data-size="modal-xl"
                level="2"
            >
                <span class="svg-icon svg-icon-white svg-icon-x">@svg('lineawesome-plus-solid')</span>
                <span>@lng('dashboard.areas.create_city','اضافة مدينة')</span>
            </a>
        </x-slot>
                    ...
                    <tr id="TR_{{$o->id}}">
                        ...
                        <td class="text-center">
                            <a  data-title="{{lng('dashboard.general.cities','عرض مدن المنطقة ')}} "
                                data-size="modal-xl"
                                level="3"
                                href="{{route('system.areas.cities',$o->parent_id)}}" class="text-gray-800 text-hover-primary mb-1 OpenModal">
                                {{@$o->parent->name}}
                            </a>
                        </td>
                        ...
                        <td class="text-center">
                            <x-theme.tables.menu :title="lng('dashboard.general.settings','الاعدادات')">
                                <x-theme.tables.menu-item :title="lng('dashboard.general.edit','تعديل')"
                                                          class="OpenModal"
                                                          data-title="{{lng('dashboard.general.edit_city','تعديل بيانات مدينة')}} "
                                                          data-size="modal-xl"
                                                          level="2"
                                                          :href="route('system.cities.update',$o->id)"/>

                               @if($o->children->count())
                                <x-theme.tables.menu-item :title="lng('dashboard.general.all_districts','عرض الاحياء')"
                                                          class="OpenModal"
                                                          data-title="{{lng('dashboard.general.districts','عرض أحياء المدن')}} "
                                                          data-size="modal-xl"
                                                          level="2"
                                                          :href="route('system.cities.districts',$o->id)"/>
