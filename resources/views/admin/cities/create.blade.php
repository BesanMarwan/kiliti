...
@section('page_title')
    <x-header.title :name="lng('dashboard.general.dashboard','لوحة التحكم')">
        ...
        <x-header.breadcrumb-item :name="lng('dashboard.cities.create_cities')"/>
    </x-header.title>
@endsection

@section('content')
    <x-theme.card>
        <form action="{{route('system.cities.store')}}" id="FormSubmit" method="post">
            @csrf
            <div class="row justify-content-center">
                ...
                <div class="col-md-6">
                    <x-inputs.select name="parent_id"  id="" required :title="lng('dashboard.general.parent_id','المنطقة')"  :options="$areas"/>
                 </div>
