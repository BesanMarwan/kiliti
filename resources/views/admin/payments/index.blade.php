...
@section('title',lng('dashboard.payments.payments','بوابات الدفع'))
@section('page_title')
    <x-header.title :name="lng('dashboard.general.dashboard','لوحة التحكم')">
        <x-header.breadcrumb-item :name="lng('dashboard.payments.payments','بوابات الدفع')"/>
    </x-header.title>
@endsection
@push('css')
    ...
    <x-theme.card>
        <x-slot name="bulkactions">
            <button type="button" class="btn btn-success mx-2 btn-bulk-action"
                    data-url="{{route('system.payments.activate')}}" data-token="{{csrf_token()}}">@lng('dashboard.general.activate_selected','تفعيل المحدد')
            </button>
            <button type="button" class="btn btn-warning mx-2 btn-bulk-action"
                    data-url="{{route('system.payments.deactivate')}}" data-token="{{csrf_token()}}">@lng('dashboard.general.deactivate_selected','تعطيل المحدد')
            </button>
        </x-slot>
                        ...
                        <td class="d-flex align-items-center">
                            <div class="symbol symbol-circle symbol-50px overflow-hidden me-3">
                                <div class="symbol-label">
                                    <img src="{{$o->icon_url}}" alt="Emma Smith" class="w-100" />
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="text-center">
                            <x-theme.tables.menu :title="lng('dashboard.general.settings','الاعدادات')">
                                <x-theme.tables.menu-item :title="lng('dashboard.general.edit','تعديل')"
                                                          class="OpenModal"
                                                          data-title="{{lng('dashboard.payments.edit_payment','تعديل بيانات بوابة الدفع')}} "
                                                          data-size="modal-xl"
                                                          level="2"
                                                          :href="route('system.payments.update',$o->id)"/>

                                @if($o->status == 'enabled')
                                    <x-theme.tables.menu-item :title="lng('dashboard.general.deactivate','تعطيل')" data-table-action="action"
                                                              data-url="{{route('system.payments.deactivate')}}"
                                ...
                                @else
                                    <x-theme.tables.menu-item :title="lng('dashboard.general.activate','تفعيل')" data-table-action="action"
                                                              data-url="{{route('system.payments.activate')}}"
                                                              data-token="{{csrf_token()}}" data-id="{{$o->id}}"/>
