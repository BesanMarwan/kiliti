...
@section('page_title')
    <x-header.title :name="lng('dashboard.general.dashboard')">
        ...
        <x-header.breadcrumb-item :name="lng('dashboard.general.edit_admin','تعديل بيانات المدير')"/>

    </x-header.title>
@endsection
@section('content')
    <form id="editForm" class="form" action="{{route('system.admins.updateAdmin',['id'=>$out->id])}}" method="post" enctype="multipart/form-data">
        @csrf
    <x-theme.card>
        ...
        <div class="row">
            <div class="col-md-9 row">
                ...
                <div class="mb-7">
                    ...
                    <label class="required fw-bold fs-6 mb-5">@lng('dashboard.general.Role','الصلاحية')</label>
                    @foreach($roles as $r)
                        <div class="d-flex fv-row">
                            <x-inputs.radio name="role_id" :value="$r->id" :checked="old('role_id',$out->hasRole($r->id))" :title="$r->name"/>
                        </div>
            ...
            <div class=" col-md-3 ">
                <x-inputs.image name="image" :value="$out->image"  :title="lng('dashboard.general.image')"/>
